...
>
  <div class="p-8 flex flex-col items-center border-b border-gray-50">
    ...
    {:else}
      <div class="relative">
        ...
        {:else if isTeacher}
          <div
            class="absolute -top-2 -right-2 bg-blue-600 text-white px-2 py-1 text-[10px] font-black rounded-lg shadow-sm"
          >
            TEACHER
          </div>
        {:else if isStudent}
          <div
            class="absolute -top-2 -right-2 bg-emerald-500 text-white px-2 py-1 text-[10px] font-black rounded-lg shadow-sm"
          >
            طالب
          </div>
        {/if}
  ...
  <nav class="flex-1 p-6 space-y-3 overflow-y-auto">
    ...
    {#if isStudent}
      <a
        href="/my-lectures"
        class="block px-4 py-3 text-gray-600 hover:text-emerald-600 font-black text-sm transition-colors"
      >
        محاضراتي
      </a>
    {/if}
